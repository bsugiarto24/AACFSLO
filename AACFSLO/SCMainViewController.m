...
@interface SCMainViewController () <CLLocationManagerDelegate, SCShareUtilityDelegate>
...
@implementation SCMainViewController
{
    ...
    __weak IBOutlet UIBarButtonItem *menuButton3;
}
...
- (void)viewDidLoad
{
    ...
    self.title = @"Log a MOI";
    
    SWRevealViewController *revealViewController = self.revealViewController;
    if ( revealViewController )
    {
        [menuButton3 setTarget: self.revealViewController];
        [menuButton3 setAction: @selector( revealToggle: )];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.shareButton.enabled = ([_selectedFriends count] > 0);
...
- (IBAction)showMain:(UIStoryboardSegue *)segue
{
    if ([_lastSegueIdentifier isEqualToString:@"showPlacePicker"]) {
        ...
        SCPickerViewController *vc = segue.sourceViewController;
        _selectedFriends = [vc.selection valueForKeyPath:@"name"];
